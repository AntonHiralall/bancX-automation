...
test.describe.skip('Document Upload Tests', () => {
    ...
    test.describe('Valid Scenarios', () => {
        test('Should successfully upload documents', async () => {
            ...
            await expect(documentUploadsPage.identityDocumentViewLocator, 'Identity document view should be visible').toBeVisible();
            await expect(documentUploadsPage.payslipViewLocator, 'Payslip view should be visible').toBeVisible();
            await expect(documentUploadsPage.bankStatementsViewLocator, 'Bank statements view should be visible').toBeVisible();
        });
    });
    ...
    test('Match help and support snapshot', async () => {
        ...
        expect(documentUploadsPage.validateSupportPageHeader(), 'Support page header should be visible').toBeTruthy();
        expect(documentUploadsPage.validateHelpAndSupportSnapshot(), 'Help and support snapshot should be visible').toBeTruthy();
    });
});
